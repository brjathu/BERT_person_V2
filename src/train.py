import pyrootutils
import torch
from src.datamodules.phalp_datamodule import PHALPDataModule
from src.models.bert_person import BERT_PERSON_LitModule
...
def train(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    # trainer: Trainer = pl.Trainer(accelerator="gpu", devices=2, strategy=DDPStrategy(find_unused_parameters=True))
    ...
    if(cfg.get("ckpt_path") is None):
        ...
        if(os.path.exists(path_tmp)):
            ...
            if(cfg.configs.wights_path is not None):
                if os.path.exists(cfg.configs.wights_path):
                    av = torch.load(cfg.configs.wights_path)['state_dict']
                    model.load_state_dict(av, strict=cfg.configs.load_strict)
                    log.info("Loading weights from weights " + cfg.configs.wights_path)
    ...
    if cfg.get("test"):
        ...
        ckpt_path = cfg.get("ckpt_path") #trainer.checkpoint_callback.last_model_path
        if ckpt_path == "":
