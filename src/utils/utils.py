...
def get_colors():    
    RGB_tuples                   = np.vstack([np.loadtxt("src/utils/colors.txt", skiprows=1) , np.random.uniform(0, 255, size=(10000, 3)), [[0,0,0]]])
...
def log_loss(opt, loss_dict, loss_name, summary_writer, step):
    ...
    for k in loss_dict.keys():
        for k_ in loss_dict[k].keys():
            loss_[k_] += loss_dict[k][k_]

    # average losses
    for k in loss_.keys():
       loss_[k] /= len(loss_dict.keys()) 

    # log losses
    for k in loss_.keys():
        summary_writer.add_scalar(loss_name + "/" + k, loss_[k], step)
        if(opt.logger=="wandb"):
            wandb.log({loss_name + "/" + k: loss_[k]}, step=step)
        print(loss_name + "/" + k + ": " + str(loss_[k]))

def log_metrics(opt, metric_dict, metric_name, summary_writer, step):
    # log metrics
    for k in metric_dict.keys():
        summary_writer.add_scalar(metric_name + "/" + k, metric_dict[k], step)
        if(opt.logger=="wandb"):
            wandb.log({metric_name + "/" + k: metric_dict[k]}, step=step)
        print(metric_name + "/" + k + ": " + str(metric_dict[k]))
