...
class SMPLHeadRE(nn.Module):
    """ SMPL Iterative Regressor with ResNet50 backbone
    """

    def __init__(self, cfg, opt):
        super(SMPLHeadRE, self).__init__()
        self.cfg             = cfg
        self.opt             = opt
        npose                = 6 * (cfg.SMPL.NUM_BODY_JOINTS + 1)
        self.npose           = npose
        in_channels          = self.opt.in_feat #226 #cfg.MODEL.SMPL_HEAD.IN_CHANNELS
    ...
    def forward(self, x, n_iter=3, smpl=False):
        ...
        # pred_body_pose_                    = copy.deepcopy(pred_body_pose)
